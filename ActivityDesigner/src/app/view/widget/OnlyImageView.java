...
public class OnlyImageView extends View{
	...
	public OnlyImageView(Context context, AttributeSet attrs) {
		super(context, attrs);
		currentStatus = CommonConst.STATUS_INIT;
	}
	...
	public boolean onTouchEvent(MotionEvent event) {
		...
		case MotionEvent.ACTION_MOVE:
			if (event.getPointerCount() == 1) {
				...
				currentStatus = CommonConst.STATUS_MOVE;
				...
				if (fingerDis > lastFingerDis) {
					currentStatus = CommonConst.STATUS_OUT;
				} else {
					currentStatus = CommonConst.STATUS_IN;
				}
				// 进行缩放倍数检查，最大只允许将图片放大4倍，最小可以缩小到初始化比例
				if ((currentStatus == CommonConst.STATUS_OUT && totalRatio < 4 * initRatio)
						|| (currentStatus == CommonConst.STATUS_IN && totalRatio > initRatio)) {
	...
	protected void onDraw(Canvas canvas) {
		...
		case CommonConst.STATUS_OUT:
		case CommonConst.STATUS_IN:
			zoom(canvas);
			break;
		case CommonConst.STATUS_MOVE:
			move(canvas);
			break;
		case CommonConst.STATUS_INIT:
			initBitmap(canvas);
		default:
